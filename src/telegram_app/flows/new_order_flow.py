...
# Paises validos como ORIGEN (nunca pseudo-destinos)
...
async def receive_client_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for fav in favorites[:7]:
        label = f"🏷️ {fav.alias} ({COUNTRY_FLAGS.get(fav.dest_country,'🌎')} {fav.dest_country})"
...
async def _try_auto_approve(
    *,
    ...
    try:
        ...
        # Estimar monto en USD (amount_origin_fiat í— rate_client ≈ USD payout)
