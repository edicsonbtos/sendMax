...
async def _prompt_for_step(update: Update, step: int) -> None:
    if step == ASK_FULL_NAME:
        await update.message.reply_text("1/6) 👤 Nombre y apellido:")
    elif step == ASK_PHONE:
        await update.message.reply_text("2/6) 📞 Teléfono (con código país):")
    elif step == ASK_ADDRESS:
        await update.message.reply_text("3/6) 📍 Dirección corta (ciudad/estado):")
    elif step == ASK_PAYOUT_COUNTRY:
        await update.message.reply_text("4/6) 🏳️ País donde cobrarás ganancias (ej: VENEZUELA):")
    elif step == ASK_PAYOUT_METHOD:
        await update.message.reply_text("5/6) 🏦 Método de pago (1 mensaje con todos los datos):")
    elif step == ASK_DOC_PHOTO:
        await update.message.reply_text("6/6) 🪪 Foto del documento (1 sola foto):")
    elif step == ASK_SELFIE_PHOTO:
        await update.message.reply_text("Último paso 😊\n🤳 Selfie con el documento a un costado:")
...
async def receive_alias(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not ALIAS_REGEX.match(alias):
        await update.message.reply_text("❌ Alias inválido. Ej: rigo_01. Intenta de nuevo:")
        return ASK_ALIAS
...
async def receive_selfie_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("✅ Verificación enviada. ⏳ Un admin la revisará.")
    return ConversationHandler.END
