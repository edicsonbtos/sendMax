...
def update_order_status(public_id: int, new_status: str) -> bool:
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (new_status, public_id))
...
# --- Awaiting paid proof (persistente) ---
...
def list_orders_awaiting_paid_proof_by(by_telegram_user_id: int, limit: int = 10) -> list[Order]:
    """Órdenes esperando comprobante de pago de destino, filtradas por el operador que marcó 'PAGADA'."""
    sql = """
        SELECT
            id, public_id, operator_user_id,
            origin_country, dest_country,
...
# --- TX helpers (sin commit): para operaciones atómicas en un mismo conn/transaction ---
